for d in "$1"/[CH]*/ ; do
  ...
  if [ sample != "CL_119_R1" ]; then
    ...
    while [ $seqsize > 0 ]; do
      ...
      echo "blast with "$nonmatchfile", output "$blastout;
      #blastx
      ...
      echo "extract ref seed seqs from "$blastout" and "$nonmatchfile", output to "$matchfile" and "$newnonmatchfile;
