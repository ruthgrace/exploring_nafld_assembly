...
if (length(args) != 2) {
  ...
  rowsums <- apply(d,1,function(x) { sum(x[c(3:(length(x)-4))]) } )
  d <- d[which(rowsums != 0),]
  print(paste("removed",length(which(rowsums==0)),"features with zero counts for all samples from",length(rowsums),"total features, leaving",which(rowsums!=0),"features left"))
